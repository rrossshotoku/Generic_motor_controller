...
typedef struct __attribute__((packed)) { uint16_t payload_crc; } MC_SpiFrameFooter_t;
...
MC_Status_t MC_Spi_EncodeFrame(uint8_t msg_type, uint16_t seq, const void *payload, uint16_t payload_len, uint8_t *frame, uint16_t *frame_len);
MC_Status_t MC_Spi_DecodeFrame(const uint8_t *frame, uint16_t frame_len, uint8_t *msg_type, uint16_t *seq, const uint8_t **payload, uint16_t *payload_len);
#endif

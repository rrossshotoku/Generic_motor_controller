...
typedef enum { MC_FAULT_SEV_WARNING, MC_FAULT_SEV_RECOVERABLE, MC_FAULT_SEV_SEVERE } MC_FaultSeverity_t;
typedef enum {
    MC_FAULT_NONE = 0,
...
typedef struct { uint32_t active_bits; uint16_t primary_error_code; bool severe_active; bool recoverable_active; bool warning_active; } MC_FaultState_t;
void MC_Faults_Init(void);
void MC_Faults_UpdateFast(void);
void MC_Faults_UpdateMedium(const MC_MechanicalState_t *state);
void MC_Faults_UpdateSlow(void);

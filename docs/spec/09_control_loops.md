...
- **Velocity loop** (D2, ADR-012): `mc_velocity_controller.c` — PI on velocity error → torque
  ...
  **Stop-integrator bleed (ADR-074):** when `vel_stop_bleed_enable` (`0x2300:13`) and the axis is
  commanded to stop (`|demand| ≈ 0`) with `|velocity| < vel_stop_bleed_v_th` (`0x2300:11`), the
  integrator is fast-unwound each tick before the PI — per-tick fraction removed `= vel_stop_bleed_factor
  (0x2300:12) · ki · dt` (a factor of ki, so 2 = twice as fast, and it auto-disables at ki = 0) — so
  its wound-up brake can't push the velocity past zero into a reverse (the on-camera recoil at the end
